# Auto-generated Terraform configuration for bbc-news-pipeline-30
...
terraform {
  ...
  #   prefix = "triggers/bbc-news-pipeline-30"
  # }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-30-ci"
  description = "CI trigger for bbc-news-pipeline-30 - runs tests on all branches"
  project     = var.project_id
  location    = var.region
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-30/**"
  ]

  filename = "pipelines/bbc-news-pipeline-30/ci.yaml"
}

# CD Trigger - runs only on default branch (master/main)
# Note: Requires GitHub repository to be connected in Cloud Build first
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-30-cd"
  description = "CD trigger for bbc-news-pipeline-30 - deploys to Dataflow"
  project     = var.project_id
  location    = var.region
  ...
  filename = "pipelines/bbc-news-pipeline-30/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-30"
