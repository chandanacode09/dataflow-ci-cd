# Auto-generated Terraform configuration for bbc-news-pipeline-35
...
terraform {
  ...
  #   prefix = "triggers/bbc-news-pipeline-35"
  # }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-35-ci"
  description = "CI trigger for bbc-news-pipeline-35 - runs tests on all branches"
  project     = var.project_id
  location    = var.region

  repository_event_config {
    repository = "projects/${var.project_id}/locations/${var.region}/connections/${var.github_connection}/repositories/${var.github_owner}-${var.github_repo}"
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-35/**"
  ]

  filename = "pipelines/bbc-news-pipeline-35/ci.yaml"
}

# CD Trigger - runs only on default branch (master/main)
# Note: Requires GitHub repository to be connected in Cloud Build first
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-35-cd"
  description = "CD trigger for bbc-news-pipeline-35 - deploys to Dataflow"
  project     = var.project_id
  location    = var.region
  ...
  filename = "pipelines/bbc-news-pipeline-35/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-35"
