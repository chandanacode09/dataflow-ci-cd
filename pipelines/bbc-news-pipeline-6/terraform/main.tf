# Auto-generated Terraform configuration for bbc-news-pipeline-6
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "dataflow-pipelines/bbc-news-pipeline-6"
  }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-6-ci"
  description = "CI trigger for bbc-news-pipeline-6 - runs tests on all branches"
  project     = var.project_id
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-6/**",
  ]

  filename = "pipelines/bbc-news-pipeline-6/ci.yaml"
...
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-6-cd"
  description = "CD trigger for bbc-news-pipeline-6 - deploys to Dataflow"
  project     = var.project_id
  ...
  filename = "pipelines/bbc-news-pipeline-6/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-6"
