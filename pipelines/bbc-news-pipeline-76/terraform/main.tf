# Auto-generated Terraform configuration for bbc-news-pipeline-76
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "dataflow-pipelines/bbc-news-pipeline-76"
  }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-76-ci"
  description = "CI trigger for bbc-news-pipeline-76 - runs tests on PRs to master"
  project     = var.project_id
  location    = var.region
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-76/**",
  ]

  filename = "pipelines/bbc-news-pipeline-76/ci.yaml"
  
  service_account = "projects/None/serviceAccounts/ci-cd-pipeline-sa@None.iam.gserviceaccount.com"

  tags = [
    "pipeline-ci",
    "bbc-news-pipeline-76",
    "auto-generated"
  ]
}
...
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-76-cd"
  description = "CD trigger for bbc-news-pipeline-76 - deploys to Dataflow"
  project     = var.project_id
  ...
  filename = "pipelines/bbc-news-pipeline-76/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-76"
