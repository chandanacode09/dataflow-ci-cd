# Auto-generated Terraform configuration for austin-311-pipeline-2
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "dataflow-pipelines/austin-311-pipeline-2"
  }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "austin-311-pipeline-2-ci"
  description = "CI trigger for austin-311-pipeline-2 - runs tests on all branches"
  project     = var.project_id
  ...
  included_files = [
    "pipelines/austin-311-pipeline-2/**",
  ]

  filename = "pipelines/austin-311-pipeline-2/ci.yaml"

  tags = [
    "pipeline-ci",
    "austin-311-pipeline-2",
    "auto-generated"
  ]
}
...
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "austin-311-pipeline-2-cd"
  description = "CD trigger for austin-311-pipeline-2 - deploys to Dataflow"
  project     = var.project_id
  ...
  filename = "pipelines/austin-311-pipeline-2/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "austin-311-pipeline-2"
