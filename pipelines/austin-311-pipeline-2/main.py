#!/usr/bin/env python3
"""
austin-311-pipeline-2 pipeline
...
class Austin311Pipeline2Options(PipelineOptions):
    """Custom pipeline options for austin-311-pipeline-2"""
...
def run(argv=None):
    """
    Main entry point for the austin-311-pipeline-2 streaming pipeline.
    """
    ...
    custom_options = pipeline_options.view_as(Austin311Pipeline2Options)

    # Construct full table references
    input_table = f"{custom_options.project}:austin_311.311_service_requests"
    output_table = f"{custom_options.project}:austin_311.processed_311_requests"
