# Auto-generated Terraform configuration for bbc-news-pipeline-52
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "dataflow-pipelines/bbc-news-pipeline-52"
  }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-52-ci"
  description = "CI trigger for bbc-news-pipeline-52 - runs tests on all branches"
  project     = var.project_id
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-52/**",
  ]

  filename = "pipelines/bbc-news-pipeline-52/ci.yaml"

  tags = [
    "pipeline-ci",
    "bbc-news-pipeline-52",
    "auto-generated"
  ]
}
...
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-52-cd"
  description = "CD trigger for bbc-news-pipeline-52 - deploys to Dataflow"
  project     = var.project_id
  ...
  filename = "pipelines/bbc-news-pipeline-52/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-52"
