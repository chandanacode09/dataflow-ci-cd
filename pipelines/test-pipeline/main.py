...
class TestPipelineOptions(PipelineOptions):
    """Custom pipeline options for test-pipeline"""
...
def run(argv=None):
    """
    Main entry point for the test-pipeline streaming pipeline.
    """
    ...
    custom_options = pipeline_options.view_as(TestPipelineOptions)

    # Construct full table references
    input_table = f"{custom_options.project}:test_dataset.source_table"
    output_table = f"{custom_options.project}:test_dataset.dest_table"
    ...
    with beam.Pipeline(options=pipeline_options) as pipeline:
        ...
        (
            ...
            | 'WriteToBigQuery' >> WriteToBigQuery(
                ...
                method='FILE_LOADS'  # Use FILE_LOADS for schema auto-detection support
            )
        )
