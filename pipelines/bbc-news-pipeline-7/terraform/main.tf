# Auto-generated Terraform configuration for bbc-news-pipeline-7
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "dataflow-pipelines/bbc-news-pipeline-7"
  }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-7-ci"
  description = "CI trigger for bbc-news-pipeline-7 - runs tests on all branches"
  project     = var.project_id
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-7/**",
  ]

  filename = "pipelines/bbc-news-pipeline-7/ci.yaml"
...
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-7-cd"
  description = "CD trigger for bbc-news-pipeline-7 - deploys to Dataflow"
  project     = var.project_id
  ...
  filename = "pipelines/bbc-news-pipeline-7/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-7"
