...
class SampleEventPipelineOptions(PipelineOptions):
    """Custom pipeline options for sample-event-pipeline"""
...
def run(argv=None):
    """
    Main entry point for the sample-event-pipeline streaming pipeline.
    """
    ...
    custom_options = pipeline_options.view_as(SampleEventPipelineOptions)

    # Construct full table references
    input_table = f"{custom_options.project}:baseball.games_wide"
    output_table = f"{custom_options.project}:baseball.processed_games_wide"
