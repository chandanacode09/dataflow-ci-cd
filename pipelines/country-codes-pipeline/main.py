...
class CountryCodesPipelineOptions(PipelineOptions):
    """Custom pipeline options for country-codes-pipeline"""
...
def run(argv=None):
    """
    Main entry point for the country-codes-pipeline streaming pipeline.
    """
    ...
    custom_options = pipeline_options.view_as(CountryCodesPipelineOptions)

    # Construct full table references
    input_table = f"{custom_options.project}:country_codes.country_codes"
    output_table = f"{custom_options.project}:country_codes.processed_country_codes"
