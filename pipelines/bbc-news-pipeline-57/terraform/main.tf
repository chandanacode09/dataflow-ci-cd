# Auto-generated Terraform configuration for bbc-news-pipeline-57
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "dataflow-pipelines/bbc-news-pipeline-57"
  }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-57-ci"
  description = "CI trigger for bbc-news-pipeline-57 - runs tests on all branches"
  project     = var.project_id
  location    = var.region
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-57/**",
  ]

  filename = "pipelines/bbc-news-pipeline-57/ci.yaml"
  
  service_account = "projects/None/serviceAccounts/@cloudbuild.gserviceaccount.com"

  tags = [
    "pipeline-ci",
    "bbc-news-pipeline-57",
    "auto-generated"
  ]
}
...
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-57-cd"
  description = "CD trigger for bbc-news-pipeline-57 - deploys to Dataflow"
  project     = var.project_id
  location    = var.region
  ...
  filename = "pipelines/bbc-news-pipeline-57/cd.yaml"

  service_account = "projects/None/serviceAccounts/@cloudbuild.gserviceaccount.com"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-57"
