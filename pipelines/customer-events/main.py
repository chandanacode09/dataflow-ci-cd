...
class CustomerEventsOptions(PipelineOptions):
    """Custom pipeline options for customer-events"""
...
def run(argv=None):
    """
    Main entry point for the customer-events streaming pipeline.
    """
    ...
    custom_options = pipeline_options.view_as(CustomerEventsOptions)

    # Construct full table references
    input_table = f"{custom_options.project}:analytics.raw_events"
    output_table = f"{custom_options.project}:analytics.processed_events"
