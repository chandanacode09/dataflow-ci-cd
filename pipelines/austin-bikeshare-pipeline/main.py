...
class AustinBikesharePipelineOptions(PipelineOptions):
    """Custom pipeline options for austin-bikeshare-pipeline"""
...
def run(argv=None):
    """
    Main entry point for the austin-bikeshare-pipeline streaming pipeline.
    """
    ...
    custom_options = pipeline_options.view_as(AustinBikesharePipelineOptions)

    # Construct full table references
    input_table = f"{custom_options.project}:austin_bikeshare.bikeshare_stations"
    output_table = f"{custom_options.project}:austin_bikeshare.processed_bikeshare_stations"
