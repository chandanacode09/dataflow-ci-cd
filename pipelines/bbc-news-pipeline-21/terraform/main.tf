# Auto-generated Terraform configuration for bbc-news-pipeline-21
...
terraform {
  ...
  #   prefix = "triggers/bbc-news-pipeline-21"
  # }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-21-ci"
  description = "CI trigger for bbc-news-pipeline-21 - runs tests on all branches"
  project     = var.project_id
  location    = var.region

  trigger_template {
    project_id  = var.project_id
    repo_name   = "${var.github_owner}_${var.github_repo}"
    branch_name = ".*"
  }

  # Only trigger if files in this pipeline changed
  included_files = [
    "pipelines/bbc-news-pipeline-21/**"
  ]

  filename = "pipelines/bbc-news-pipeline-21/ci.yaml"
}

# CD Trigger - runs only on default branch (master/main)
# Note: Requires GitHub repository to be connected in Cloud Build first
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-21-cd"
  description = "CD trigger for bbc-news-pipeline-21 - deploys to Dataflow"
  project     = var.project_id
  location    = var.region
  ...
  filename = "pipelines/bbc-news-pipeline-21/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-21"
