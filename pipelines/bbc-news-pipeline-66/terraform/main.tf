# Auto-generated Terraform configuration for bbc-news-pipeline-66
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "dataflow-pipelines/bbc-news-pipeline-66"
  }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-66-ci"
  description = "CI trigger for bbc-news-pipeline-66 - runs tests on PRs to master"
  project     = var.project_id
  location    = var.region
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-66/**",
  ]

  filename = "pipelines/bbc-news-pipeline-66/ci.yaml"
  
  service_account = "projects/None/serviceAccounts/ci-cd-pipeline-sa@None.iam.gserviceaccount.com"

  tags = [
    "pipeline-ci",
    "bbc-news-pipeline-66",
    "auto-generated"
  ]
}
...
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-66-cd"
  description = "CD trigger for bbc-news-pipeline-66 - deploys to Dataflow"
  project     = var.project_id
  ...
  filename = "pipelines/bbc-news-pipeline-66/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-66"
