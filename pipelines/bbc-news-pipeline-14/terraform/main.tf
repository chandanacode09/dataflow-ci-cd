# Auto-generated Terraform configuration for bbc-news-pipeline-14
...
terraform {
  ...
  #   prefix = "triggers/bbc-news-pipeline-14"
  # }
}
...
resource "google_cloudbuild_trigger" "ci_trigger" {
  name        = "bbc-news-pipeline-14-ci"
  description = "CI trigger for bbc-news-pipeline-14 - runs tests on all branches"
  project     = var.project_id
  ...
  included_files = [
    "pipelines/bbc-news-pipeline-14/**",
  ]

  filename = "pipelines/bbc-news-pipeline-14/ci.yaml"

  tags = [
    "pipeline-ci",
    "bbc-news-pipeline-14",
    "auto-generated"
  ]
}
...
resource "google_cloudbuild_trigger" "cd_trigger" {
  name        = "bbc-news-pipeline-14-cd"
  description = "CD trigger for bbc-news-pipeline-14 - deploys to Dataflow"
  project     = var.project_id
  ...
  filename = "pipelines/bbc-news-pipeline-14/cd.yaml"

  substitutions = {
    _PIPELINE_NAME = "bbc-news-pipeline-14"
