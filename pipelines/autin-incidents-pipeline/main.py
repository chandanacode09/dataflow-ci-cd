...
class AutinIncidentsPipelineOptions(PipelineOptions):
    """Custom pipeline options for autin-incidents-pipeline"""
...
def run(argv=None):
    """
    Main entry point for the autin-incidents-pipeline streaming pipeline.
    """
    ...
    custom_options = pipeline_options.view_as(AutinIncidentsPipelineOptions)

    # Construct full table references
    input_table = f"{custom_options.project}:austin_incidents.incidents_2008"
    output_table = f"{custom_options.project}:austin_incidents.processed_austin_incidents"
